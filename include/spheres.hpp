...
namespace spheres {
	...
	public:
		...
		void choice(bool gravitational) {
			initialize();
			...
			for (int j = 0; j < 3; j++) {
				if (random((int) (settings::mul / screen::lengh[0])) % 2 == 0) {
					...
					if (!gravitational) {
						velocity.coord[j] = -random((int) (settings::mul / screen::lengh[0]));
					}
				}
				else {
					position.coord[j] = -random((int) (settings::mul / screen::lengh[0]));
		...
		void getacel(Sphere other) {
			quantities::Vector soft = quantities::Vector(0.001, 0.001, 0.001);
		...
		void collision(spheres::Sphere other) {
			if (!(position - other.position) <= radius + other.radius) {
				...
				quantities::Scalar resm = mass + other.mass;
				quantities::Scalar two = quantities::Scalar(2);

				velocity -= dist * ((resv || dist) / (!dist * !dist)) * (two * other.mass / resm);
				aceleration -= dist * ((resa || dist) / (!dist * !dist)) * (two * other.mass / resm);
			}
		}
